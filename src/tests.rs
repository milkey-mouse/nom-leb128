    ...
    ($int_ty:ident, $encoder:ident, $decoder:ident, $iter:expr) => {
        ...
        fn $int_ty() {
            for n in $iter {
                ...
                let decoded = $decoder::<_, ()>(encoded.as_slice()).unwrap();
...
macro_rules! test_tricky_unsigned {
    ($int_ty:ident, $decoder:ident) => {
        ...
        test_roundtrip!($int_ty, write_unsigned_leb128, $decoder, [
...
macro_rules! test_tricky_signed {
    ($int_ty:ident, $decoder:ident) => {
        test_roundtrip!($int_ty, write_signed_leb128, $decoder, [
            0, 1, 128, 255, 256, 65535, 65536, $int_ty::MAX,
            -1, -128, -255, -256, -65535, -65536, $int_ty::MIN,
        ].iter().copied());
    };
}
